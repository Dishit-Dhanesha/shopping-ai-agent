...
async function getIngredientsFromLLM(recipe) {
    ...
    const systemPrompt = `You are a JSON-only API. Extract essential ingredients for recipes and return ONLY valid JSON.

Response format:
{
  "recipe": "recipe name",
  "ingredients": [
    { "name": "ingredient", "quantity": "amount", "essential": true }
  ]
}

Rules:
- Maximum 6 ingredients with essential: true
- Use simple ingredient names (e.g., "chicken", "tomatoes", "pasta")
- Skip common items like salt, water, oil unless they are special/essential
- Keep quantities simple (e.g., "500g", "2 cups", "1 bunch")
- Return ONLY valid JSON, no other text`;

    const response = await model.invoke([
        { role: "system", content: systemPrompt },
        new HumanMessage(`Recipe: ${recipe}`)
    ]);
    ...
    try {
        ...
        if (!parsed.recipe || !Array.isArray(parsed.ingredients)) {
            throw new Error('Invalid JSON structure: missing recipe or ingredients array');
        }
        ...
        // Fallback: try to construct a basic response
        throw new AppError(
